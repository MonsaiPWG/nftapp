...
def generate_nft(nft_number, assets, full_set_prob, shiny_prob):
    # Determinar si se activa full set y shiny weapon
    ...
    # Las categorías de traits que usaremos
    ...
    if is_full_set and available_sets and len(available_sets) > 0:
        ...
        for trait in trait_keys:
            # Para "clothing", se usará el nombre "ARMOR" en la convención
            if trait == "clothing":
    ...
    if random.random() < shiny_prob:
        ...
        if weapon_fname in assets["shiny_weapon"]:
            # Si se encuentra la versión shiny, se utiliza en lugar de la normal
            selected_traits["weapon"] = weapon_fname
            is_shiny = True

    # ▸ Crear la imagen final combinando las capas en orden:
    # 1. Fondo
    nft_img = assets["background"].copy()
    # 2. Weapon (se pega POR DEBAJO del cuerpo)
    ...
    # 4. Otros traits en orden fijo (puedes modificar este orden según necesites)
    ...
    # ▸ Construir la metadata con atributos
